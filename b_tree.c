...
int setValToNode(int val,int *pval,struct btreeNode *node,struct btreeNode **child){
    int pos;
    if(!node){
        *pval=val;
        *child=NULL;
    ...
    if(setValToNode(val,pval,node->link[pos],child)){
        if(node->count<MAX){
...
void insertion(int val){
    ...
    flag=setValToNode(val,&i,root,&child);
    if(flag){
